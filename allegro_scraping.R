...
# TREŚĆ z moodle:
# Zapisać dane z ogłoszeń ze strony: https://www.otomoto.pl/ (lub innej)
# Każdy wektor z danymi zapisać do pliku:
# write.table(rbind(wektorDanych), file="ogloszenia.csv", row.names=FALSE, col.names=FALSE, sep="\t", append=TRUE, na="NA")
# Należy pamiętać o wyczyszczeniu/usunięciu go jeżeli taki plik istnieje.
# Do znalezienia miasta z ogłoszeń z mieszkaniami wykorzystałem plik z data-framem z miastami mlocalload ("mlocal.rd")
#mlocal <- mlocalload('mlocal.rd')
#listsplit <- strsplit('mlocal.rd', ", ")
#for( li in 1 : length(listsplit[[1]] )){
#  #print(paste0("Miasto z ",li," - ",listsplit[[1]][li]))
#  if (tolower(listsplit[[1]][li])%in%mlocal$Miasto) {
#    lokalizacja<-tolower(listsplit[[1]][li])
#    print(lokalizacja)
#    break
#  }  
#  lokalizacja <- NA
